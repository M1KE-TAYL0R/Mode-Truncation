...
for etai in range(len(eta)):
    ...
    for j in range (e_length):
        for m in range (ns):
            if etai != 0:
                if m != j:
                    E_new[j+1, etai] += (dse_tot[m,j] + coup_tot[m,j])**2 / (result[etai][0][j] - result[etai][0][m])
                    # E_new[j+1, etai] += (coup_tot[m,j])**2 / (result[etai][0][m] - result[etai][0][j])
...
E_new[0,:] = eta
#--------------------------------------------
np.savetxt(f"E-f{nf}-n{ns}-p{points}_abs_perturb.txt", E_new.T)
# np.savetxt(f"DSE-f{nf}-n{ns}-p{points}.txt", dse_cont.T)
# np.savetxt(f"Coup-f{nf}-n{ns}-p{points}.txt", coup_cont.T)