    ...
    for j in range (20):
        ...
        for m in range (20):
            if  etai != 0:
                if m != j:
                    Em = E_eta[m]
                    E_new[j+1, etai] += (dse_tot[j,m] + coup_tot[j,m])*(dse_tot[m,j] + coup_tot[m,j]) / (Ej - Em)
                    # E_new[j+1, etai] += (coup_tot[j,m])*(coup_tot[j,m]) / (Ej - Em)
                    dse_cont[j+1,etai] += (dse_tot[j,m])*(dse_tot[m,j]) / (Ej - Em)
                    coup_cont[j+1, etai] += (coup_tot[j,m])*(coup_tot[j,m]) / (Ej - Em)
...
E_new[0,:] = eta
#--------------------------------------------
np.savetxt(f"E-f{nf}-n{ns}-p{points}_abs_p.txt", E_new.T)
np.savetxt(f"DSE-f{nf}-n{ns}-p{points}_p.txt", dse_cont.T)
np.savetxt(f"Coup-f{nf}-n{ns}-p{points}_p.txt", coup_cont.T)
np.savetxt(f"Add-f{nf}-n{ns}-p{points}_p.txt", coup_cont.T + dse_cont.T)